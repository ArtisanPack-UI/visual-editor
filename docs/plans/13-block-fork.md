...
- **Grid split shape** — I3 splits the group block's `grid` variation into standalone `artisanpack/grid` + `artisanpack/grid-item` blocks (parity-plus customization). **Basis:** the existing Crosswinds Blocks `basic-grid` / `basic-grid-item` pair ([repo](https://gitlab.com/jacob-martella-web-design/crosswinds-framework/crosswinds-framework-plugins/crosswinds-blocks/-/tree/main/blocks/src)) — open to refactoring to fit our stack. Key decisions it already resolves:
  - **`grid-item` is an explicit parent-locked child** (`"parent": ["artisanpack/grid"]`), not an auto-wrapped inner block.
  - **Responsive column model** on the parent: `numDesktopColumns` / `numTabletColumns` / `numMobileColumns` (defaults 4/2/1), plus `gridSameGap` toggle and `gridGap` / `gridColumnGap` / `gridRowGap`. Parent exposes column counts to children via block context (`providesContext` → `usesContext`).
  - **Child span attributes** per breakpoint: `gridColumnSpan{Desktop,Tablet,Mobile}` + `gridRowSpan{…}`, plus an `innerLayout` string.
  - Refactor checklist when porting: bump `apiVersion` 2 → 3; replace `__experimental*` supports (`__experimentalLayout`, `__experimentalBorder`, `__experimentalFontFamily`, …) with their stabilized equivalents; renamespace to `artisanpack/*` + textdomain `artisanpack-visual-editor`; drop the WP `index.js`/`save` in favor of the Blade/React/Vue renderer trinity; pick our own block category.
