...
- **H8 — Dev-app sample theme.** ✅ Shipped. `themes/dev-sample/` carries the minimal fixture (templates / parts / synced + unsynced patterns / global styles / menu locations). Smoke flow documented at [`docs/h8-smoke-flow.md`](../h8-smoke-flow.md); version-pair compatibility documented in the package README. Decided at H8 kickoff: minimal `dev-sample` (not a fork of `digital-shopfront`) — Keystone CMS + `jmwd-default` cover real-consumer dogfooding, `dev-sample` is the regression net inside this repo.
...
Plan 11 Phase D legacy code is gone. The six `VisualEditor{Template,TemplatePart,Pattern,PatternCategory,GlobalStyles,Navigation}` models + their five policies + their eight migrations + the orphaned `MenuLocationResolver`, `GlobalStylesCssProvider`, `GlobalStylesCacheInvalidator`, `GlobalStylesCompiler`, and legacy `Resources/TemplateResolver` are all deleted. `TemplatePartInliner` + `PatternInliner` were rewritten to consume cms-framework's resolvers under `class_exists` guards; `EntitySearchController` now drives template / template-part search through cms-framework's resolvers; `MenuLocationsController` reads cms-framework's `MenuLocationAssignment` directly. The renderer-blade package's `BlocksComponent` + `TemplateComponent` swap the deleted CSS provider for a thin `GlobalStylesEmissionResolver` that delegates to cms-framework's `GlobalStylesEmitter`. A drop-migration retires the orphan `visual_editor_*` tables on installs that ran the old migrations.
...
- ~~**H8 sample theme name + scope** — fork cms-framework's `digital-shopfront` reference theme into the dev-app, or build a minimal `dev-sample` theme just for the smoke flow? Decide at H8 kickoff.~~ Resolved at H8 kickoff: minimal `dev-sample`. Reasoning: Keystone + `jmwd-default` already serves as the real-consumer dogfooding surface; the in-package theme stays small so smoke-flow regressions point at the right place rather than getting lost in production styling.
