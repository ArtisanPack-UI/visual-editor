 ...
 *
 * Mirrors `packages/visual-editor-renderer-react/src/blocks/artisanpack/socialIcons.ts`
 * and the Blade `SocialIconRegistry` so every renderer emits the same
 * chip markup for the same `(slug, url)` pair.
 */
