...
<div class="min-h-screen bg-base-200">
	<div class="max-w-6xl mx-auto px-6 py-12">
		...
		<div class="flex flex-wrap items-center gap-3 mb-6">
			...
			{{-- Area filter --}}
			<select wire:model.live="filterArea" class="select select-bordered select-sm">
			...
			<div class="overflow-x-auto">
				<table class="table table-zebra w-full">
					<thead>
						<tr>
							<th class="w-10">
								<input type="checkbox" class="checkbox checkbox-sm" wire:click="toggleSelectAll" @checked( count( $selected ) > 0 && count( $selected ) === $parts->count() ) />
							</th>
								...
								<td class="text-right">
									...
									<div class="flex items-center justify-end gap-1">
										...
										@if ( $part->is_locked ?? false )
											<button class="btn btn-ghost btn-xs" wire:click="toggleLock( {{ $part->id }} )" title="{{ __( 'visual-editor::ve.listing_unlock' ) }}">
												<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
											</button>
										@else
											<button class="btn btn-ghost btn-xs" wire:click="toggleLock( {{ $part->id }} )" title="{{ __( 'visual-editor::ve.listing_lock' ) }}">
