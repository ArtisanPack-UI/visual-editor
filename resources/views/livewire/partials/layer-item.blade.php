...
@if ( $itemCount > 0 )
	{{-- Grid items drag context with lifecycle management --}}
	<div
		wire:key="layer-grid-items-drag-container-{{ $blockId }}"
		x-drag-context
		...
		x-data="{
			...
			init() {
				...
				Livewire.hook('morph', ({ component, cleanup }) => {
					if (this.isDragging) {
						console.log('Layers: Preventing morph during grid drag');
						return false;
					}
				});

				console.log('Layers: Grid items drag context initialized for {{ $blockId }}');
			}
		}"
		@dragstart.capture="
	...
	@drag:end="
		(() => {
			...
			console.log('🔵 LAYERS: drag:end fired', {
				blockId: '{{ $blockId }}',
			...
			if (newOrder.length > 0 && newOrder.every(idx => !isNaN(idx))) {
				...
				console.warn('❌ LAYERS: Invalid grid item order, skipping reorder:', newOrder);
			}
		})()
	"
		@drag:cross-context="
			(() => {
				console.log('🔵 LAYERS: Grid cross-context handler FIRED', {
					sourceContext: $event.detail.sourceContext.getAttribute('wire:key'),
					targetContext: $event.detail.targetContext.getAttribute('wire:key'),
					itemId: $event.detail.itemId
				});
				...
				const sourceMatch = sourceKey.match(/^layer-grid-items-drag-container-(.+)$/);
				const targetMatch = targetKey.match(/^layer-grid-items-drag-container-(.+)$/);
				...
				console.log('🚀 Calling $wire.dispatchCrossContextGridItemMove with:', {
					sourceParentId,
					sourceItemIndex,
					targetParentId,
					movedItemIndex
				...
				$wire.dispatchCrossContextGridItemMove(
					sourceParentId,
					sourceItemIndex,
					targetParentId,
					movedItemIndex
	...
	@for ( $itemIndex = 0; $itemIndex < $itemCount; $itemIndex++ )
		@php
			...
			$itemId       = $item['id'] ?? $blockId . '-item-' . $itemIndex;
			$gridItemId   = $blockId . '-item-' . $itemIndex;
			$isItemActive = $gridItemId === $activeColumnId;
		@endphp

		{{-- Grid item - draggable for reordering --}}
		<div
			x-drag-item="'{{ $gridItemId }}'"
			wire:key="layer-item-{{ $blockId }}-{{ $itemIndex }}"
			wire:click="selectColumn( '{{ $blockId }}', {{ $itemIndex }} )"
			class="flex w-full cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm transition-colors
				{{ $isItemActive ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700 hover:bg-gray-100' }}"
			role="listitem"
			style="padding-left: {{ ( $depth + 1 ) * 1.25 }}rem;"
		>
			<x-artisanpack-icon name="fas.grip-vertical" class="h-3 w-3 shrink-0 cursor-grab text-gray-300" />
			<x-artisanpack-icon name="fas.table-cells-large" class="h-4 w-4 shrink-0 {{ $isItemActive ? 'text-blue-500' : 'text-gray-400' }}" />
