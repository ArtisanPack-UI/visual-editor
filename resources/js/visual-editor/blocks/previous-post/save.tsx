 ...
 * persisted inner-block tree against the previous adjacent post and
 * emits the final markup. But the block hosts inner blocks inside a
 * wrapper `<div>` (see `edit.tsx`), so `save()` must reproduce that
 * exact wrapper — mirroring the editor's `useBlockProps` className —
 * or Gutenberg's validator flags the saved markup as "unexpected or
...
export default function PreviousPostSave(): ReactElement {
