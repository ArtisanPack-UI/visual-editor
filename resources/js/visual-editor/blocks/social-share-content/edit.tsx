/**
 * Social Share Content — editor-side component (#501).
 *
 * Authors pick which platforms to expose, the chip style, the layout
 * direction, and how the chips stretch in their container. The canvas
 * renders a stub preview without real share URLs — server-side
 * `PostResolver` stamps `_resolvedShareLinks` against the current post
 * so each chip points at the right pre-filled share endpoint.
 */
...
interface SocialShareContentAttributes {
...
interface SocialShareContentEditProps {
    readonly attributes: SocialShareContentAttributes;
    readonly setAttributes: (
        next: Partial<SocialShareContentAttributes>
    ) => void;
}
...
function isSharePlatformSlug(slug: string): boolean {
    return (SHARE_SOCIAL_PLATFORM_SLUGS as ReadonlyArray<string>).includes(slug);
}
...
export default function SocialShareContentEdit({
    attributes,
    setAttributes,
}: SocialShareContentEditProps): ReactElement {
    const selected = Array.isArray(attributes.socialIcons)
        ? attributes.socialIcons.filter(
              (slug): slug is string => typeof slug === 'string' && isSharePlatformSlug(slug)
          )
        : [];
        ...
        return (
            <div className="ap-social-share-content__item" key={platform.slug}>
                ...
                >
                    ...
                    {showLabel && (
                        <span className="ap-social-share-content__label">
                            {platform.label}
                        </span>
                    )}
                </span>
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody
                    title={__('Social share settings', TEXT_DOMAIN)}
                    initialOpen
                >
                    {shareSocialPlatforms().map((platform) => (
                        <CheckboxControl
                            key={platform.slug}
            ...
            <div {...blockProps}>
                {selected.length === 0 ? (
                    <p className="ap-social-share-content__empty">
                        {__(
