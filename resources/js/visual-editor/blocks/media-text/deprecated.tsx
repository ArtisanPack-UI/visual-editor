/**
 * Media & Text — deprecation chain.
 *
 * Ported from `@wordpress/block-library/src/media-text/deprecated.js`
 * (v9.43.0). Upstream ships seven deprecation entries (v1 … v7). All
 * entries are preserved here verbatim under the new namespace so legacy
 * `core/media-text` markup deserializes cleanly after a `core/media-text`
 * → `artisanpack/media-text` round-trip via the block transforms in
 * `transforms.ts`.
 */
...
interface BackgroundFillStyles {
    backgroundImage?: string;
    backgroundPosition?: string;
}

const v1ToV5ImageFillStyles = (
    url?: string,
...
const migrateCustomColors = (attributes: AnyAttributes): AnyAttributes => {
    ...
    const { customBackgroundColor: _omit, ...restAttributes } = attributes;
    return {
        ...restAttributes,
        style,
    };
...
const migrateDefaultAlign = (attributes: AnyAttributes): AnyAttributes => {
    if (attributes.align) {
        return attributes;
    }
...
const v7 = {
    ...
    save({ attributes }: SaveArgs): ReactElement {
        const {
            isStackedOnMobile,
            mediaAlt,
            mediaPosition,
        ...
        const mediaSizeSlug =
            (attributes.mediaSizeSlug as string | undefined) ||
...
const v6 = {
    ...
    isEligible(
        attributes: AnyAttributes,
        _innerBlocks: unknown,
        { block }: { block: { attributes: AnyAttributes } }
    ): boolean {
        ...
        return (
            ...
            !!(finalizedAttributes.className as string | undefined)?.includes(
                'alignwide'
            )
        );
    },
